In our example, we want to backup the _**\/var\/www**_ folder from a GNU\/Linux client that has the 192.168.1.12 IP address.
...
But when ? It depends on its **_Policy_** configuration. We have assigned the **_Default Policy_** to this job, so if we see how is it configurated we could see which are it's _**schedule**_ and _**retention policy**_.
